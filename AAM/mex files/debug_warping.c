...
int numberOfPointsMapped=0,waste;
mxArray *pauseVal=mxCreateDoubleMatrix(1,1,mxREAL);
double *pause_val=mxGetPr(pauseVal);
*pause_val=3;
i=1;
j=63;
cartesianToBarycentric(BarycentricCoordinates,points,points_x,points_y,connectivityList,connectivityList_x,k,i,j);
printf("\n %d  %d %f %f %f",i,j,BarycentricCoordinates[0],BarycentricCoordinates[1],BarycentricCoordinates[2]);
return ;
for (i=0;i<Texture_m;i++)
{
  for(j=0;j<Texture_n;j++)
  {
	...
	barycentricToCartesian(BarycentricCoordinates,triangleVertices,cartesian);
       ...
       ptr[j*Texture_m+i]=1;
       printf("\n %d  %d %f %f %f",i,j,BarycentricCoordinates[0],BarycentricCoordinates[1],BarycentricCoordinates[2]);
      mexCallMATLAB(0,NULL,1,&pauseVal,"pause");
       break;
      }
    }  
    count++;
